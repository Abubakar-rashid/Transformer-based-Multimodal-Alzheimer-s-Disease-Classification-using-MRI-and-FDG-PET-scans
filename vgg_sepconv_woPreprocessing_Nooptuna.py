...
def make_single_loaders_backbone(df, batch_size, img_size, name):
    """
    For backbone training: train/val/test
    Test uses overlap subjects
    """
    loaders = {}
...
# ==============================================================
# 4. SEPARABLE CONVOLUTION BUILDING BLOCK
# ==============================================================
class SeparableConv2d(nn.Module):
    """
    Depthwise-separable convolution: depthwise Conv2d followed by a
    pointwise (1×1) Conv2d.  Bias is placed only on the pointwise layer,
    matching standard Conv2d behaviour.
    """
...
def _make_sepconv_vgg19_features():
    """
    Rebuild the VGG19 feature block with SeparableConv2d instead of Conv2d.
    The architecture (channels, strides, pooling) is identical to torchvision VGG19.
    Weights are initialised from scratch (no pretrained separable-conv weights exist
    for VGG19, so we start from ImageNet-pretrained pointwise initialisations would
    require a custom loader — here we simply use random init and let the network
    train end-to-end).
    """
    # VGG19 feature configuration:
    # numbers = out_channels, 'M' = MaxPool2d(2,2)
    cfg = [64, 64, 'M',
           128, 128, 'M',
           256, 256, 256, 256, 'M',
           512, 512, 512, 512, 'M',
    ...
    for v in cfg:
        if v == 'M':
            ...
            layers.append(nn.BatchNorm2d(v))   # BN stabilises training from scratch
...
# ==============================================================
# 4. MODEL ARCHITECTURE (separable-conv VGG19)
# ==============================================================
class VGG19Extractor(nn.Module):
    """
    VGG19-style extractor where every Conv2d is replaced by a
    depthwise-separable convolution.  BatchNorm is added after
    each separable block to compensate for the lack of pretrained
    ImageNet weights.
    """
...
def measure_inference_time(model, test_df, img_size, name):
    ...
    Processes ONE subject at a time to avoid OOM (subjects are NOT pre-loaded
    all at once).  GPU timing uses torch.cuda.Event; CPU uses perf_counter.
    ...
    if has_gpu:
        ...
        for subj in subjects:
            ...
            end_event.record()
            torch.cuda.synchronize()
            gpu_times[subj] = start_event.elapsed_time(end_event)   # ms
            del batch
    ...
    for subj in subjects:
        ...
        cpu_times[subj] = (t1 - t0) * 1000.0   # ms
        del batch
    ...
    for subj in subjects:
        ...
        results.append({"subject": subj, "n_slices": ns,
                        "gpu_ms": gpu_ms if has_gpu else None,
                        "cpu_ms": cpu_ms})
    ...
    # Restore model to GPU if available
    if has_gpu:
...
def measure_inference_time_multimodal(model, mri_test_df, pet_test_df, img_size, name):
    ...
    Processes ONE subject at a time (streaming) to avoid OOM — the Cartesian
    product of MRI×PET pairs can be very large when pre-loaded for all subjects.
    """
    print(f"\n{'='*70}")
    ...
    def _load_subject(subj):
        """Build MRI×PET Cartesian-product tensors for one subject (CPU)."""
    ...
    if has_gpu:
        ...
        for subj in common:
            ...
            mri_b = mri_b.to(gpu_device)
            pet_b = pet_b.to(gpu_device)
            start_event = torch.cuda.Event(enable_timing=True)
            end_event   = torch.cuda.Event(enable_timing=True)
            start_event.record()
            _ = model_gpu(mri_b, pet_b)
            end_event.record()
            torch.cuda.synchronize()
...
def main():
    ...
    # ── STAGE 1: MRI VGG19-SepConv ────────────────────────────
    print("\n" + "="*60)
    print("STAGE 1 — MRI VGG19-SepConv")
    ...
    mri_model, mri_hist = fit(
        ...
        save_path = os.path.join(OUT_DIR, "mri_vgg19_sepconv_best.pth"),
    ...
    measure_inference_time(
        ...
        CFG["img_size"], "MRI_VGG19_SepConv")

    # ── STAGE 2: PET VGG19-SepConv ────────────────────────────
    print("\n" + "="*60)
    print("STAGE 2 — PET VGG19-SepConv")
    ...
    pet_model, pet_hist = fit(
        ...
        save_path = os.path.join(OUT_DIR, "pet_vgg19_sepconv_best.pth"),
    ...
    measure_inference_time(
        ...
        CFG["img_size"], "PET_VGG19_SepConv")
    ...
    print("  VGG19-SepConv backbones frozen — only fusion head trains")
    print("="*60)

    # Reload best backbone weights and freeze them
    mri_model.load_state_dict(torch.load(
        os.path.join(OUT_DIR, "mri_vgg19_sepconv_best.pth"), map_location=DEVICE))
    pet_model.load_state_dict(torch.load(
        os.path.join(OUT_DIR, "pet_vgg19_sepconv_best.pth"), map_location=DEVICE))
    ...
    mm_model, mm_hist = fit(
        mm_model, mm_loaders,
        save_path = os.path.join(OUT_DIR, "multimodal_sepconv_best.pth"),
    ...
    measure_inference_time_multimodal(
        ...
        CFG["img_size"], "Multimodal_VGG19_SepConv")
    ...
    print(f"  {'-'*75}")
    print(f"  {'MRI VGG19-SepConv':<20} {len(mri_bb_test_subjs):<15} {mri_acc:>10.4f} {mri_su_acc:>10.4f} {mri_su_roc:>10.4f}")
    print(f"  {'PET VGG19-SepConv':<20} {len(mri_bb_test_subjs):<15} {pet_acc:>10.4f} {pet_su_acc:>10.4f} {pet_su_roc:>10.4f}")
    ...
    torch.save(mm_model.state_dict(),
               os.path.join(OUT_DIR, "multimodal_sepconv_final.pth"))
